...
module.exports = {
  ...
  async execute(message, param){
    ...
    if (!permissions.has('CONNECT') || !permissions.has('SPEAK')) return message.channel.send('You are missing the connect, speak or both permissions');
    ...
    if (cmd === 'play'){
        if (!args.length) return message.channel.send(":thinking: Please give a song name or youtube URL. I can't figure it out on my own");
        let song = {};
        ...
        if (ytdl.validateURL(args[0])) {
            ...
            if (video){
                song = { title: video.title, url: video.url }
                  ...
                  message.channel.send("Sorry couldn't find the song");
            }
        }
        ...
        if (!server_queue){
            ...
            return message.channel.send(`:clipboard: **${song.title}** added to queue!`);
        }
    }
